...
#include "../../../src/applicationInternal/hardware/arduinoLayer.h" // for millis()
      ...
      case SDL_MOUSEBUTTONDOWN:
        ...
        for (auto const &key : manager->keys)
        {
          ...
          if (pnpoly(key.num_vert, key.vertx, key.verty, mouse_event->x, mouse_event->y))
          {
            ...
            holdTimer = SDL_MOUSEBUTTONDOWN ? millis() : 0;
            break;
          }
        }
    } 
...
KeyState pumpKeys() {
  ...
  if (activeKey.state == GUI_KEY_PRESSED && (millis() - holdTimer) > KEY_HOLD_TIME) {
